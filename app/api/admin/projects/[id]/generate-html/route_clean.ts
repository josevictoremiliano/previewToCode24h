...
export async function POST(
  ...
  try {
    ...
    const templateVariables = {
      ...
      brandColors: project.briefing?.brandColors || `${project.data?.visualIdentity?.primaryColor || '#3B82F6'}, ${project.data?.visualIdentity?.secondaryColor || '#1E40AF'}`,
      style: project.briefing?.style || project.data?.visualIdentity?.style || 'moderno',
      additionalRequirements: project.briefing?.additionalRequirements || '',
      slogan: project.data?.basicInfo?.slogan || '',
      siteType: project.briefing?.businessType || project.data?.basicInfo?.siteType || '',
      niche: project.data?.basicInfo?.niche || '',
      products: project.briefing?.mainServices || '',
      cta: project.data?.content?.cta || 'Entre em contato',
      sections: project.data?.content?.sections ? (Array.isArray(project.data.content.sections) ? project.data.content.sections.join(', ') : project.data.content.sections) : 'hero,sobre,servicos,contato',
      primaryColor: project.data?.visualIdentity?.primaryColor || '#3B82F6',
      secondaryColor: project.data?.visualIdentity?.secondaryColor || '#1E40AF',
      customTexts: project.data?.additionalResources?.customTexts || '',
      features: project.data?.additionalResources?.features ? (Array.isArray(project.data.additionalResources.features) ? project.data.additionalResources.features.join(', ') : project.data.additionalResources.features) : '',
      email: project.data?.contact?.email || project.briefing?.contactInfo || '',
      phone: project.data?.contact?.phone || '',
      address: project.data?.contact?.address || '',
      socialMedia: project.data?.contact?.socialMedia ? JSON.stringify(project.data.contact.socialMedia) : ''
    }
